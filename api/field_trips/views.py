...
def login(request):
    ...
    response.set_cookie(
        "auth_token", token.key, httponly=True, samesite="None", secure=True
    )
    return response
...
def signup(request):
    ...
    if username in settings.ACCOUNT_USERNAME_BLACKLIST:
        return Response(
            {"detail": "This username is not allowed."}, status.HTTP_400_BAD_REQUEST
        )
...
def logout(request):
    if request.user.is_authenticated:
        ...
        response = Response(
            "logged out: {}".format(request.user.email), status=status.HTTP_200_OK
        )
...
def trip_list(request, format=None):
    ...
    if request.method == "POST":
        ...
        if not (
            ...
            and IsLeaderOrCoordinator().has_permission(request, None)
        ):
...
def trip_registration(request, trip_id):
    if request.method == "GET":
        is_registered = Registration.objects.filter(
            user=request.user, trip_id=trip_id
        ).exists()
        ...
        data.update({"user": request.user.id, "trip": trip.id})
...
# health-check endpoint to query DB
@api_view(["GET"])
@permission_classes([AllowAny])
def health_check(request):
    Trip.objects.all().count()  # Forces database query
    return Response(
        {
            "status": "ok",
